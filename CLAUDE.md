...
both. Dark mode is not covered by the axe/contrast tests.
...
- Unit: `tests/*.test.mjs`, `npm test` (~10s, 185).
- Browser: `e2e/`, one file per surface, 546 across two projects —
  `daily`, `index`, `saint`, `chrome`, `quality-floor`, plus `helpers.js`.
