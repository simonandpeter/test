 ...
 * for one — and opens the chooser in a panel beneath the header. The
 * disclosure itself — the flight, the outside press, Escape — is
 * ui/panel-control.js, shared with the language control since 2026-09-05;
 * this owns the button's words and the panel's contents. Returns the
 * unsubscribe.
 */
export function mountChurchControl(button, panel) {
  ...
  const control = mountPanelControl(button, panel, {
    render: () => renderChooser(),
    wire: (root) => wireChooser(root, { onChange: () => control.close() }),
  });
