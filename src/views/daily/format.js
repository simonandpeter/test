 ...
 *
 * Two `Intl` formats, one reckoning and one parse, shared by everything that
 * prints a day: the heading, the rail's weekday, and the prose that names a
 * date inside a sentence. They are here rather than in `lib/` because
 * the choices in them are this page's — `reckonedPlain` spells the month out
 ...
 */
const reckonedDate = (iso, calendar, options) => {
...
/** A date named inside a sentence, in a chosen calendar: day, month in full, year. */
 ...
 * outright. `reckonedPlain(iso, 'gregorian')` is identity on a civil date, so
 * this needs no branch for that case either.
 *
 ...
 * calendar than the one they are reading.
 */
