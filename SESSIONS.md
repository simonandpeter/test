...
108 amendments. 1 to 41 are bold-numbered paragraphs in the
...
| 99 | [the map cut into a folder, the two big specs cut by surface, and a checker taught about spreads](#amendment-99-the-map-cut-into-a-folder-the-two-big-specs-cut-by-surface-and-a-checker-taught-about-spreads-2026-09-05-evening) | 2026-09-05, evening |
| 100 | [the queue of one-line lives closed from azbyka.ru, and the date audit read down](#amendment-100-the-queue-of-one-line-lives-closed-from-azbykaru-and-the-date-audit-read-down-2026-09-05-night-to-2026-09-06) | 2026-09-05 night to 2026-09-06 |
| 101 | [past the queue: saint.gr behind its fold, Butler's Western saints, and eighty-nine short Sretensky lives tried against azbyka](#amendment-101-past-the-queue-saintgr-behind-its-fold-butlers-western-saints-and-eighty-nine-short-sretensky-lives-tried-against-azbyka-2026-09-06-afternoon-and-evening) | 2026-09-06, afternoon and evening |
| 102 | [the All Saints carousel: what it fetches, and how the columns are packed](#amendment-102-the-all-saints-carousel-what-it-fetches-and-how-the-columns-are-packed-2026-09-06-into-the-7th) | 2026-09-06 into the 7th |
| 103 | [Amendment 2 reversed for hymns, the Related lists derived, and Continue reading on a phone](#amendment-103-amendment-2-reversed-for-hymns-the-related-lists-derived-and-continue-reading-on-a-phone-2026-09-07) | 2026-09-07 |
| 104 | [Texts in the header, the carousel filling in, the atlas set in capitals, the rails splined, and eight more journeys](#amendment-104-texts-in-the-header-the-carousel-filling-in-the-atlas-set-in-capitals-the-rails-splined-and-eight-more-journeys-2026-09-07-evening) | 2026-09-07, evening |
| 105 | [the header row moved to the rung it fits on, the tone put into the reader's language, and a language audit](#amendment-105-the-header-row-moved-to-the-rung-it-fits-on-the-tone-put-into-the-readers-language-and-a-language-audit-2026-09-07-night) | 2026-09-07, night |
| 106 | [the phone's nav made endless in fact, the second Anthony linked, and the offices put into the reader's language](#amendment-106-the-phones-nav-made-endless-in-fact-the-second-anthony-linked-and-the-offices-put-into-the-readers-language-2026-09-08) | 2026-09-08 |
| 107 | [the lifespans translated, the strip full while it moves, and the carousel's lag measured before it was guessed at](#amendment-107-the-lifespans-translated-the-strip-full-while-it-moves-and-the-carousels-lag-measured-before-it-was-guessed-at-2026-09-08-evening) | 2026-09-08, evening |
| 108 | [the strip answers the press rather than the navigation, and the atlas gets out of the corpus's way](#amendment-108-the-strip-answers-the-press-rather-than-the-navigation-and-the-atlas-gets-out-of-the-corpuss-way-2026-09-08-late) | 2026-09-08, late |
...
## Amendment 99 — the map cut into a folder, the two big specs cut by surface, and a checker taught about spreads (2026-09-05, evening)

The author's word, in full: "Items 5 and 6" — the two entries of
`docs/CLEANUP-PLAN.md` that the plan had said wanted the author's word before
anyone started, given after Amendment 98's content sweep had closed. Both are
done; the plan is finished.

**Item 5: `views/map.js` (4,603 lines) is `views/map/`.** The file keeps 676
lines — the markup, `render`, `destroy`, and the choosing of a saint or a blob
— and the rest is seven modules: `state.js` (306), `paint.js` (2,159),
`motion.js` (376), `timeline.js` (482), `chrome.js` (476), `search.js` (195),
`press.js` (157). The question the plan left open — what to do with the
forty-odd module-scoped `let`s that the draw pass and every wiring function
read and write — was answered the way `views/daily/state.js` and
`views/index/state.js` answered it: one object, in a leaf module, that every
other module reads and writes and none copies. Two differences from those
two, both said in the file: it is a constant object with fields rather than a
per-render object opened and closed, because most of what the map holds is
deliberately *kept* across visits (the range, the triangle, Movement, the
speed, the filter boxes) and only some of it is reset by `render` and
`destroy`; and it is named `map` rather than `state`, because `state` is what
forty places in the paint pass call a dot's own `live`/`past`/`future`, and a
shared object of that name would have been shadowed in every one of them.
`frameOf`, `ceilingOf`, `place`, `shownYear` and `announce` live in the leaf
too, because every module needs them and the leaf must import none of them.
The terrain loaders stayed in `paint.js` on purpose: they call `paintCanvas`
when a tile lands and it calls them when a frame needs one, so a `terrain.js`
would import its importer.

The cut was a script, not a hand edit: each top-level declaration with the
comment above it was a unit, each unit was assigned a module by name, the
twenty shared variables became `map.<field>` by a regex over code lines only,
and every rewritten line was printed for reading (178 of them). Reading found
two things the regex had got wrong, both in the markup's own HTML comments —
prose, not JS comments, so the "skip comment lines" rule did not protect it —
and one it had missed: `...view`, because a spread's three dots end in a dot
and the rewrite treated anything after a dot as a property. All three fixed
before the first build. Four dead names went with the cut — `kind`, `KINDS`,
`DEFAULT_KIND`, `pointsOfKind`, the kind selector's leftovers, unreferenced
since the buttons went on 2026-08-31 — and `MERGE_PX` is imported for the
first time: `paintCanvas`'s centre test used it in the one- or two-member
blob branch with no import at all, a `ReferenceError` waiting since
2026-09-04 for a blob too small to have a hull to be the open one.

**What `extraction-check.mjs` said, and what it could not.** Run over the cut
(CLAUDE.md trap 6) it reported 29 names, every one a documented false
positive — `map-movement` reads as `movement`, `data-speed` as `speed`, the
new object's own keys read as bare uses in `state.js`, and `export … from`
is not an `import` so `searchMatches` looked stranded — confirmed by a grep
for bare state names in code lines that found only string literals. Then the
build passed, `node --check` passed on every file, and the map spec went red
142 of 160: `ReferenceError: clampCentre is not defined` in `defaultView`.
The same spread blindness, one level down: `...clampCentre(x, y)` sat behind
three dots in `map.js` and `search.js`, the import generator's lookbehind
read the last of them as a member access, and **so did the checker's** — its
`(?<![.\w$])` had exactly the exclusion its own header warns about, and the
symptom was exactly the silence it describes. The checker now excludes only a
dot that is not itself preceded by a dot; pointed at the pre-fix `search.js`
it names `clampCentre`. That is the fourth time the file's own history
records it being wrong by excluding a candidate, and it is written there.

**Item 6: the two largest specs are six.** `daily.spec.js` (5,792 lines, 129
tests) is `daily-panel.spec.js` (79 tests — the hero, the date line and its
chips, the fast and its bubble, the readings, the hymns, the feasts, the two
columns, the way into the life), `daily-picker.spec.js` (43 — the week strip
and rail, the month, the full-screen calendar, the reckoning, the keys and
swipes that turn the day) and `daily-register.spec.js` (7 — the
also-commemorated cards and the name days). `index.spec.js` (4,894 lines, 92
tests) is `index-carousel.spec.js` (28), `index-grid.spec.js` (28) and
`index-controls.spec.js` (36). By surface, as the plan asks and not by date:
each test was classified from the selectors and helpers it uses, and the
seam is a judgement the file headers say so about — a test that reads two
surfaces sits with the one its title names. The tests are untouched. The
`---- round ----` dividers, which are the rounds the tests were written in
and the suite's provenance, are repeated in whichever file holds a member of
that round; the tests above the first divider are the ones written before
the file had any. Each file imports only the helpers it uses; `dealtOrder`,
the one module-scope helper, went with its three tests to the carousel file.
The checker reports nothing left behind on either split, and
`playwright test --list` counts 442 tests in the six files — 221 × 2
projects, the same as before.

**Docs.** CLAUDE.md's map section says what the folder is and the rule that
makes it safe; its surface-to-spec table names the six files and
`views/map/*`; the `COLD_FACE=1` example and `fixtures.js`'s copy of it
point at `index-grid.spec.js`, which holds the test that pins its own face;
five passages that located a map function by file now name the module it
is in (`warmTerrainTiles`, `zoomFade`, `rankOf` in `paint.js`, the
two-pointer branch in `chrome.js`, the terrain-import trap). HANDOFF.md's
cleanup line and its `undated` audit row, `chrome.spec.js`'s cross-reference
to the Julian note, and `docs/CLEANUP-PLAN.md` (items 5 and 6 marked done
with the numbers, the intro rewritten) follow.

**Verification.** Unit suite 323 green after every step. The map spec
locally, both projects: the first run after the cut went 142 red of 160
with the `clampCentre` error above — the build, `node --check` and the
checker had all passed, and only the browser said otherwise, which is why
trap 6's recipe ends in "suite" and not in "build". After the fix, 157
green and 3 red, all three 30-second timeouts in the minutes a dev server
was also up for a smoke test; all three green in 6–8 s each run alone,
HANDOFF.md's own load signature for this file (the run's wall clock is the
predictor, not the diff). The whole suite locally at six
workers, both projects: 875 green, 5 red in 10.9 minutes — the same three
map tests, one more on the phone project (`the map opens on the coarse
coastline…`) and one axe scan in vigil mode, every one a 30–35 s timeout.
Measured rather than blamed (2026-09-02's own lesson): the map spec at the
same six workers on the *pre-split* `map.js`, stashed back for the purpose,
went 157 green and 3 red, two of them the very same tests (`the land keeps
its own ink…`, `panning the crowd off screen…`) and the third their
neighbour in the blob round — the load, not the diff. The phone coastline
test and the whole vigil-mode axe set green alone, 12 s and 44 s. CI's
verdict follows in the next amendment or in HANDOFF.md, whichever comes
first.

## Amendment 100 — the queue of one-line lives closed from azbyka.ru, and the date audit read down (2026-09-05 night to 2026-09-06)

The author's word, after Amendment 99 had closed the cleanup plan: "keep
working on saint profiles" — the standing instruction of Amendment 98,
resumed. Six commits between `a7795a7` and `c889e20`, 75 lives and
52 `saint.json` files, and the whole of it is content.

**The queue.** Amendment 98 left 106 lives whose only source was the
Sretensky calendar's own line — folders whose life said "the calendar links
no written life, so this entry records the line and nothing more" — and
listed them with their civil dates and Russian names
(`scratchpad/thin_sretensky.txt`). They are all read now. The road to each
was the same: the azbyka.ru day page for the civil date lists every saint of
the day with a `/days/sv-…` link, the life page is sliced out of
`document.body.innerText` between "Дни памяти" and "Развернуть" in
16,000-character slices (the Glinsk pages have no "Дни памяти" and are cut
from the `h1`), and the life is written from the slice with the source line
naming azbyka, where it was read, whatever fond.ru or pstgu.ru page azbyka
itself reprints. By date: the twenty-three of 22 September — the synaxis of
the Glinsk fathers (Basil Kishkin, Philaret Danilevsky, Theodotus, Macarius,
Martyrius, Euthymius, Dositheus, Iliodor, Innocent, Luke, Archippus,
Joannicius Gomolko, Seraphim Amelin, Andronicus Lukash, Zenobius Mazhuga,
Seraphim Romantsov) and the day's new martyrs; the thirteen of 23 September
(Tatiana Grimblit, Bishop Uar of Lipetsk and eleven priests); the thirteen
of 28 September (the four Kuban priests shot in one night in 1921, the
Volokolamsk protodeacon Nicholas Tsvetkov, the archimandrite Ignatius
Biryukov's history of Valuyki, the archpriest John Ilyinsky shot on the feast
of the Cross); and the singletons and pairs of 6, 8, 9, 18, 21, 24, 25 and
26 September. Two of the pages were joint lives (Peter Yurkov with his
warden Simeon Turkin; John Maisuradze with George Mkheidze of Betania), and
each saint got his own life from it with a link to the other.

**Two apologies retired.** "Runs on past the point read here" (18 lives,
Amendment 98's unfinished tail) and "read only so far" (9 lives: the
Ferapontov company of 1918 under the joint azbyka life of Barsanuphius of
Kirillov, Pitirim of Perm, Macarius the Protopsaltes) are both gone from the
corpus; every one was re-read to its end. Pitirim's life now prints the two
death years its two sources give, 1455 and 1456, rather than choosing.
`alexander-deacon-martyr-1918` is Alexander (Ipatov) now, azbyka's spelling,
where the Sretensky line had been read as Ilatov.

**The audit.** `no-death` 23 → 7: sixteen saints had the year in the very
life they cited and it is recorded, with the basis copied from the birth's
(Amphilochius of Pangarati 1570, Dionysius Erhan 1943, Ilarion Felea 1961,
Joachim of Alexandria 1567, Joasaph of Belgorod 1754, Joseph the New of
Partos 1656, Meletius of Myoupolis 1105, Mitrophan of Voronezh 1703,
Onuphrius of Vorona 1789, Romylos of Ravanica 1375, Theodosius of Totma
1568, Anthimos of Cephalonia 1782, Cyriacus of Tazlau c. 1660, Daniel
Katounakiotis 1929, Gerasimus of Sourvia 1740, Macarius the Protopsaltes
1836). The seven left have no year in anything read. `no-birth` 490 → 457:
the new martyrs' pages state a birth date almost without exception, and
thirty-five were recorded as they were read. `undated` 150 → 148: two
entries that exist for the *finding* of relics — Nicholas of Alma-Ata,
Alexander Urodov — had neither year, and both are in their lives. Sabbas of
27 August stays undated on purpose: azbyka names him "of Venefaly" and says
the place and time are unknown, which is the Greek calendar's Venetala by
the look of it and is recorded as a resemblance, not an identification.

**What is still thin.** Nine lives in the corpus still say "records the line
and nothing more", none of them Russian: Albeus, Kiaran of Clonmacnoise and
Ninian of Whithorn, Edith of Wilton, Amphilochius Makris of Patmos, Isaac and
Martin of 22 September, Pimen the martyr, Zer-Jacob of Abyssinia. That is
the next queue; azbyka carries many Western saints and saint.gr the Greek
one, so the same pipeline should reach most of them.

**Tooling found, and written into CLAUDE.md's azbyka bullet.**
`browser_batch` refuses more than 25 actions, and a result past roughly 25k
tokens is saved to a file rather than returned; `scratchpad/splitbatch.py`
cuts such a file into one text file per page, and the pages were read in
chunks under ~10k characters because the Bash tool's own output also
persists past ~30 kB. A page longer than a slice was fetched again from a
fixed offset (`t.slice(a+16000, …)`) rather than from a remembered phrase.
The Nativity of the Theotokos day page (21 September) lists no saints at
all, and the two Georgian confessors were reached by guessing
`sv-ioann-majsuradze` and `sv-georgij-mheidze`, which was right. The
`display_name` is the heading and nothing else: the folder
`evdokia-tkachenko` displays "Eudocia", and `tests/lives.test.mjs` is what
would have caught a heading that followed the slug.
...
## Amendment 101 — past the queue: saint.gr behind its fold, Butler's Western saints, and eighty-nine short Sretensky lives tried against azbyka (2026-09-06, afternoon and evening)

The same standing word, "keep working on saint profiles", after Amendment
100 had closed the one-line queue and its postscript had read seven of the
nine non-Russian stragglers. Four more commits, `dc030ec`, `645f1a3`,
`fe94a3a` and `2eb7fd6`: forty-one lives rewritten from a fuller source
and twenty-two `saint.json` files given a year they lacked.

**Page length was a false signal.** With no queue left, the next question
was which of the corpus's *short* lives had a fuller source already named
in their own source line. `scratchpad/sgscan.py` fetched the saint.gr page
of every entry that cites one (179 of them, the bio cut between
" Βιογραφία" and "Λειτουργικά κείμενα") and ranked them by how much longer
the Greek ran than the English. The ranking was nearly useless: almost every
page over the 700-character line was already whole in the entry, the Greek
simply being longer than its English. What did pay was reading for
*structure* rather than length — pages whose seventy-word fold hid a whole
life, and pages that cross-refer to another saint's day where the life
actually is. Those gave Nektarios of Aegina (2960), Symeon the Lesbian
stylite (883, and his years, 765–766 to 844), Pope Martin (261, died 655),
Paisios the Athonite, Hilarion the New of Crete, Paul the New of
Constantinople, Nicodemus the Hagiorite, Ambrose of Moschonisia, the sons
of Eustathius, and Theodore Hadji of Mytilene, whose own day's page (2351)
carries what the joint page (3563) only points at. The lesson for the next
scan is in the file: measure the fold and the cross-references, not the
byte count.

**Butler, volumes VII and VIII.** Amendment 100's postscript opened Alban
Butler's *Lives of the Saints* (1866, on Bartleby) for three Irish and
English saints; the same road gave Ebba the Elder, Sebbi of Essex,
Cuthburga and Victor of Marseilles their first real lives. Butler names his
own sources (Bede, Malmesbury, the Bollandists) and the entries carry them;
the source line's form is Amendment 100's. That is the whole of what the
West's one-line entries could reach this way — the remaining thin lives
were Sretensky's, not Butler's.

**The second queue: eighty-nine short Sretensky lives.** `scratchpad/thin2.txt`
lists every life under about 140 words whose only source is the Sretensky
calendar's own printed life, with the civil date and the Russian name, the
same shape as Amendment 100's queue. The road was the same too — azbyka's
day page for the civil date, then the page it links — and it forked at
once. The Russian saints gained, and gained a great deal: Job of Pochaev
and Daniel of Moscow, whose entries had been relics-day notes, are whole
lives now, with births of about 1551 and 1261; Alexander Peresvet and
Andrew Oslyabya; the new martyrs of 20 September (Michael Tikhonitsky's
paschal night at the Orlov cemetery and his execution there, Basil
Sungurov, Leo Egorov, Peter Snezhnitsky — four births); Gorazd of Bohemia,
whose azbyka life is the whole fight for an Orthodox Czech Church and the
Heydrich crypt (born 1879); Aithalas the deacon; George I of
Constantinople and the council of 680; the four shot with Metropolitan
Eugene (Zernov) at Karaganda on 20 September 1937 (Eugene Vyzhva, Nicholas
Ashchepyev, Pachomius Ionov, Stephen Kreydich), who share the metropolitan's
page, which says of them only that they died with him — and each entry
says exactly that rather than borrowing his life; and the four Serbian and
Ural entries that had carried two calendar lines and nothing else: Raphael
of Šišatovac from the Syrmian martyrology (born 1875), Peter Ievlev of
Nevyansk from the PSTGU database (born 1875; the Sretensky name page's
"Iovlev" is a variant the diocesan gazette never used, and the entry keeps
its heading and says so), Peter Zimonić of Dabar-Bosnia (born 1866, and a
death the page honestly leaves between Jadovno and Jasenovac), and the
synaxis of Jasenovac, which azbyka tells through the one martyr it names,
Vukašin of Klepci, in the executioner's own words. That page carries a
long polemical history of the Croatian state alongside the martyrdom; the
entry keeps the martyrdom and the Church's own acts and leaves the polemic
where it was.

**The Roman-era companies did not gain, and the reason is structural.**
azbyka's day page lists a company under its leader only — Susanna's
companions, Capito and Pamphilus, Abundius and Irenaeus, Memnon and
Severus, Faustus's company, the Nicomedia companies under Anthimus and with
Cyprian, Babylas's three children and their mother, Hermas, Polyaenus and
Serapion, Emilian's companions, the Gaza three, Paul and Juliana of
Ptolemais, Thyrsus, Leucius and Coronatus, Anthousa's household — and the
leader's page reprints the very Sretensky life the entries already
summarise, in the same words. Cheremon, Phantinus, Joannicius II, Rufina
and Theodotus, the 3,628 of Nicomedia, Raisa and Sarbel were checked
page by page and are the same case. Those entries are as full as their
sources; a new source, not a new site, is the only way further. One of
the checks corrected an error rather than adding a fact: azbyka's only
Martha is the mother of Symeon the *Younger*, of the Wonderful Mountain,
died 551 and kept on 4 July, and the entry for the elder Symeon's mother
had read the Greek synaxarion's 4 July Martha as the same woman. It no
longer does.

**The audit, at close.** `undated` 145 (148 at Amendment 100: Amphilochius
of Patmos, Symeon the Lesbian stylite and Pope Martin are dated from
saint.gr's whole pages), `no-death` 6 (Kiaran's is from the OCA),
`no-birth` 448 (457; the nine are named above). The empty-range test's
window moved a fifth time with the Romylos date, which the postscript to
Amendment 100 records, and its tray count is 147.

**CI.** `224fc75` is the last green run at the time of writing (880 passed,
...
## Amendment 102 — the All Saints carousel: what it fetches, and how the columns are packed (2026-09-06 into the 7th)

A brief written for a fresh chat, in two parts and both about the same row.
"Images load too slowly: it takes a long time for all the icons to appear ...
load what is on screen first, prefetch the next cells in the scroll direction,
don't fetch every thumb at once, use the thumb files (not icon.jpg), lazy/async
decoding, and keep the endless loop smooth while images land." And: "Saints
with icons are too sparse: sometimes five columns in a row show names only ...
at least every second column contains a saint with an image, and make the
text-only saints more compact ... so the strip reads as pictures with names
between them rather than long stretches of text." Two commits, `308b5d5` and
`72ef198`.

**The bench had to be fixed before any of it could be believed.** The first
run of `scratchpad/cx-measure.mjs` said the old row delivered 11.9 MB of icons
in eleven seconds over a throttled 1.6 Mbit/s link and never showed a blank
card, which is not a thing that link can do. `Network.emulateNetworkConditions`
is set on the *page's* CDP session and a service worker fetches from a target
of its own, so once `sw.js` had activated and taken the page every icon arrived
at LAN speed with the throttle still nominally in force. `serviceWorkers:
'block'` on the context is the fix, and it is also the honest case to measure:
a first visit has no worker in charge yet. The numbers below are all from after
that. **This is the first of three instruments in one sitting that read as
evidence and were not**, which is CLAUDE.md's own standing warning met three
more times in a day.

The second was the travel. The bench stepped a whole window of the row at a
time, which flatters the arrangement it was meant to judge: teleporting past
360 px gives a picture no warning at all, so the only strategy that can ever
win is fetching the entire band at once — the thing being replaced. It travels
smoothly now, at two speeds worth telling apart: the drift's own 26 px/s, where
a blank card is a real defect, and `loopScroll`'s 900 px/s wheel cap, where on
a 4G link there are more pictures per second than the pipe can carry however
they are ordered, so what is being measured is how gracefully the row falls
behind.

**The row was fetching `icon.jpg` itself.** A median of 283 kB and a maximum of
1.04 MB, 42.3 MB across the 130 icons, to draw a picture 150 px wide on a
phone. The author's parenthesis — "use the thumb files" — could not be taken
literally: `image.lqip` is `make_thumbs.py`'s quarter-scale blurred placeholder
and is visibly soft at any real card size, which is what a placeholder is *for*
(rendered side by side at 150 px and DPR 3 before the decision was made, which
is what settled it). So the script writes a second derivative, `icon-card.jpg`,
sharp and capped at 560 px on its long edge at quality 74 — enough for the
widest card the row draws at two device pixels, and for a phone's 150 at three
— which is a median of 49 kB, 6.4 MB across the corpus. The manifest carries it
as `image.card` and the build fails on a folder missing it, the same way it
already failed on a missing thumb. `image.src` is untouched, which is what the
saint's own page and the hero still draw; `w`/`h` stay the original's, because
their job is the aspect ratio that reserves the box and the derivative
preserves it exactly. WebP was measured and refused: 5.3 MB against 6.4, not
worth a second format in a pipeline that has one.

**And the band was a starting gun rather than a queue.** `windowImages` handed
every picture inside the prefetch margin its source in one observer callback,
so the browser shared one connection between the two icons a phone can see and
the six behind them. It sorts now — on screen, then the side the row is
travelling toward, then the side behind it, nearest first within each — and
holds four in flight. `fetchpriority` is high for what is on screen and low for
everything else, which was **measured rather than reasoned**: the argument for
giving the leading band `auto` (a reader is about to want it, and the hint is
never revised mid-flight) is true and still loses, 20% of on-screen icons
painted against 32% under a hard fling, five runs each, because a prefetch at
equal priority competes for the one connection with the picture the reader is
looking at now.

The heading comes from `loopScroll.direction()` rather than from the track's
own `scrollLeft`, which is corrected by a whole period every so often and whose
deltas therefore lie; every place that moves the row records its own sign. A
finger on a phone moves it through the `scroll` handler and nowhere else — this
file refuses to write under a live touch — so that is the only place a touch's
direction can be read, and a delta of about a whole period is a correction
rather than a traveller and says nothing about who is going where.

| 360 px, throttled 4G | before | after |
| --- | --- | --- |
| the row on screen | 2,573 ms | 2,576 ms |
| every on-screen icon painted | 12,234 ms | 3,783 ms |
| the first screenful | 11 req / 3,468 kB | 6 req / 192 kB |
| icons painted under a hard fling | 16% | 92% |

At 1280: 9,973 ms to 4,116, 13 requests and 1,746 kB to 10 and 264, and 60% to
100% under the fling. The row's own arrival is unmoved because it is the boot's
cost and no picture's — the manifest, the entry bundle and the packing — which
is the floor the second number is measured against.

**The third false instrument was mine, and it is the one worth keeping.**
`settle` was registered as a `load` listener but written to take the `<img>`,
so what it was handed was the `Event`, `img.removeEventListener` threw before
`running` was decremented, and the queue never freed a slot again: four
pictures out, and no fifth. It hid on a phone, where the row's own drift
carries cards out of the band and `release` calls the same function *with an
image* — the queue was being unwedged from the side, by movement, sixty times a
minute — and it showed at a desk under `reducedMotion`, which is where the
suite holds the row still in order to read the order. The fling figures above
are the fix; the ones in `308b5d5`'s own message are the wedged version and are
worse.

### The packing

130 of 862 have an icon, so packing in the reader's own order deals them where
they fall: measured over the whole run, five columns in a row with no picture
at 360 px and three at 1280, which is exactly the report. `carouselCells`
decides a column's *kind* before it fills it — a **picture column**, seeded
with an imaged saint and drawn at `--cx-w`, or a **name column**, which takes
imageless saints only and is drawn at `--cx-w-text`. A saint with a picture is
never put in a name column even where the room would take one: the column is
narrow, and the same icon drawn at two widths in one row reads as a mistake.
Nothing is lost, because the cursor finds them again.

Two rules keep the guarantee and each was arrived at by having the other one
fail. The **floor** — a column following one with no picture reaches for one —
came first, on its own, and produced a row whose last fifth had no icons left
at all: alternation spends one icon per two columns *and never more*, so a
column seeded on whatever the cursor happened to be became a picture column
whenever the cursor stood on an imaged saint, and the icons were spent as fast
as they were met. A long stretch of text moved to the end of a looping row is
still a long stretch of text, met every time round. So the seed of a name
column reaches *past* the pictures, and a **pace** was added: each column earns
its share of the icons still unplaced over the columns still to come, and takes
one as soon as it has earned a whole one. `perColumn` is the run's own average
depth so far rather than a constant, because how many saints a column holds is
what the window's height and the reader's own filter decide.

The pace then broke the floor, which is the part worth remembering: it spends
icons at very nearly the rate the corpus supplies them — one saint in seven,
about five to a column — and running a supply at its own limit means local
droughts. The reach found nothing often enough to leave three columns of names
in a row. Where a column is *owed* a picture the reach is unbounded now, so a
drought is filled by pulling one saint forward; where it is merely surplus,
`LOOKAHEAD` still binds and the reader's order is left alone.

**A caption's height is counted per saint.** It was one of two constants — 94
on a phone, 64 at a desk — chosen against the corpus's worst caption, because
under-estimating pushes the last card of a column past the fold and only that
error is visible to a reader. The cost was the other half of the report: the
common caption is 46, so a column of names was mostly air, five names where
eight would fit. `nameLines` moved from `views/index/grid.js` to
`lib/name-lines.js` for both views to read — the arithmetic alone; each view
keeps its own probe, because what decides where a name breaks is whatever the
browser resolved for that view's own type.

**Both caps written into that estimate were the same mistake, twice.** The
first held the subtext to three lines, and "St John", whose caption is one line
of name over four of "Metropolitan of Kyiv · Reposed 1089", was budgeted 78 px
and drew 90. The second held the whole estimate under the old flat constant, on
the reasoning that a number proven safe as an upper bound could only be safe as
a ceiling — but 94 was the worst caption in *Literata*, and the runner's own
face draws one at 96. A ceiling below the truth is an under-estimate wearing a
safety belt. Nothing is capped now; the stylesheet clamps neither line, so
nothing should pretend it does. `CAPTION_SLACK` is four pixels kept back per
card, which is the honest residue: a canvas `measureText` and a laid-out line
box do not agree to the pixel, and where the estimate is low it is low by about
one — which did not matter while the slack was tens of pixels and does when the
errors are per card and a column is eight deep.

**`data-h` is what made any of that findable.** The column's fit is arithmetic
done before anything renders, and the only way to interrogate it was `the
carousel fits the window at every size`, which catches a column that overflowed
— but only once it has overflowed by enough to push a scrollbar past the page's
own 64 px of bottom padding, and which says nothing about *which* card was
mis-budgeted. The estimate is written onto each card now, beside the box the
browser drew. It found the four-line subtext in one reading, after two rounds
of adding slack by guess.

| over the whole run | 360 before | 360 after | 1280 before | 1280 after |
| --- | --- | --- | --- | --- |
| longest run of columns with no picture | 5 | 2 | 3 | 1 |
| columns carrying a picture | 54% | 74% | 68% | 76% |
| icons in a screenful, mean | 1.8 | 2.4 | 3.5 | 4.4 |
| icons in a screenful, worst | 0 | 1 | 1 | 3 |

The two at 360 is the tail and not a failure of the rule: when the last icon
has been placed, whatever imageless saints remain have nowhere to go but a
column of their own. `a picture stands in every second column at least` pins 2
rather than 1 for that reason — pinning 1 would be pinning the corpus's own
ratio of icons to saints, which the next folder changes.

**The name column is narrower at a desk and not on a phone**, 186 px against
300 and 150 either way: a phone's column is already as narrow as these names
stand, and taking it further wraps every one of them to four lines. It is a
class rather than an inline width — `index.css` derives `--cx-w-text` from
`--cx-w` and `.cx-cell.is-names` redefines `--cx-w` for its own subtree, which
all four of `.cx-card`, `.cx-media`, `.cx-name` and `.cx-sub` already read. A
pixel written from script would be the previous window's, since the build is
skipped when the packing has not changed; that is the bug `--cx-fill` carries a
paragraph about, and this is the same trap declined.

Two existing tests had to be told that the row has two widths now. `a carousel
card is half again as wide on a desktop` and `a carousel card is sized by the
window height as well as its width` both read the first `.cx-card` or `.cx-cell`
in the DOM, which is a name column as often as not; both name
`:not(.is-names)` now, because the instruction each pins was about the
*images*. `a carousel card prints the whole name` asserted one width for every
card and asserts one width per kind. None of the three claims changed.

**Five back-outs.** The derivative (reverted to `image.src`), the cap and the
ordering (the old all-at-once, which fetches an off-screen picture before one
on screen), the heading (pinned forward, which fails the backward leg twice in
two), the alternation (6 and 7 barren columns return) and the counted caption
(four names where the flat budget allowed four). The narrow name column too,
which is six. 890 pass locally with 0 flaky, and the cold-face rehearsal is
clean on the three Index specs.

**A second test fails under `COLD_FACE=1` and only under it**, found here and
confirmed on the unmodified tree before being recorded: `a card prints the whole
name, however many lines it takes, and a row still does not` reads two lines
where it wants more than two, in both projects. It is the second standing trap
applied to a line *count* rather than a width — "Righteous Theodulus,
executioner converted by Hermione" takes three lines in Literata and two in the
rehearsal's Times. CLAUDE.md records it beside the saint.spec.js one.

**CI.** `308b5d5` was green: 886 passed in 16.3 minutes, 0 flaky, accessibility
100 on all four Lighthouse pages and FCP 1357–1370 ms — All Saints itself at
1370 against the §13 floor of 1500, which is its own baseline of 1356–1372 and
not a millisecond of the gate spent. HANDOFF.md carries the verdict of
`72ef198`'s run.

**What was deliberately not done.** The Index *grid* still draws `image.src` at
full size behind `loading="lazy"`, and it is one line and the obvious next
place for the derivative; it was left because the brief named the carousel.
`sw.js`'s image cache is capped at 150 entries and now has three files per
...
put back to the author before anything could be done about it: "go through the
following saints in particular: St John the long Suffering, St Moses the
Hungarian, St Anthony of the Kiev Caves, St Theodosius, and make sure they all
have troparion and kontakion in English. Also make sure any saints mentioned in
their life have a hyperlink and are in the 'related' subheading section of the
profile. Then also add the 'continue reading' section to the bottom of every
saint profile page on mobile. Then go through all the other saints…"

### The rule, and the reversal

`schema/saint.schema.json` had said it in its own words since 2026-08-26:
`english` is "a published English rendering of this same hymn, where one exists
in a source this site may copy … **Not a translation made here — Amendment 2
forbids that** — and not a different church's hymn."

Counted before answering: **427 hymn objects across 140 saints, of which 39
carry an English rendering — all of them troparia, none a kontakion, every one
from Orloff's *General Menaion* (1899) or Hapgood's *Service Book* (1906).** And
a hymn belongs to a church, so those two Slavonic-tradition books can never
reach the **149 Greek and 117 Romanian** hymns at all: the apolytikion and the
tropar are different texts, not translations of one. The instruction as written
was unreachable for 62% of the corpus's hymns under the rule, and the honest
thing was to say so rather than to do the reachable third quietly and call it
done.

Three ways were put: stay inside the rule and take what Orloff still allows;
widen the sources, which needs a living translator's permission and is the
author's to obtain; or reverse Amendment 2 for hymns. **The author chose the
reversal**, and `docs/HYMN-PLAN.md` — written before the question was asked,
with the counts and the formula — records what each option would have cost.

**Reversed, not absorbed, and it came with a condition.** `english` is now
exactly one of two things and has to say which: `source`, somebody else's
published rendering with its citation, or `rendered: "site"`, this site's own.
The schema refuses both and refuses neither — a `oneOf` with a paired `not`,
proved against ajv's 2020-12 build before it was believed — and `ui/hymns.js`
prints the difference: **"Rendered for this site"** where a citation would name
its book, with no link, because there is no book. A reader must be able to tell
a translation made here from a text copied out of one, and that is the whole of
what the reversal is worth.

Nothing else in the corpus is translated and none of this licenses it. The
lives stay English by decision (Amendment 2's own first case) and the About page
still says why.

The locale packs took `calendar.hymns.renderedHere` in all four, and
`about.sourcing.texts` — which had been falling back to English in all four
since it was added — went in with it. 476 of 476 again.

### The four saints

**Anthony and Theodosius of the Kyiv Caves share one office**: the same four
hymns stand in both folders, two troparia and two kontakia for the two founders
together. None is a common hymn Orloff prints, so no citation was ever going to
reach them. Rendered in the register of the two books already cited, so that a
page showing one of each does not read as two different sites; the Slavonic's
`/` and `//` half- and full-verse marks are dropped, as Orloff's own prose drops
them.

**John the Long-Suffering and Moses the Hungarian carried no hymn in any
language**, so the first work on them was not English at all: five texts read
from azbyka's own pages through the Browser pane (403 to Python and curl —
CLAUDE.md's own bullet), John's troparion and kontakion and Moses's two troparia
and kontakion, the second of Moses's being what azbyka prints as "Ин тропарь".
**The modern Russian gloss azbyka prints beside each hymn is not copied and the
English is not made from it**: the Slavonic is the text, and a rendering of a
rendering is two removes from what the church sings. Moses's second troparion
turns out to be the reason he is in this corpus at all, put into words — taking
hunger, imprisonment and wounds, he was not made a slave to the Polish woman's
pleasures, which is the whole of the five years his `track` walks between 1018
and 1025.

### The Related lists

The hyperlinks were never the gap. `lib/cross-link.js` has linked a recognised
name in a life since 2026-08-26 and `cross-link-audit.mjs` prints all 86 of
them; **`related` was a hand list that exactly one folder in the corpus had
filled in**. So the obvious move was to derive one from the other, and the
obvious move was wrong: **twenty-three of the 86 are a church, a lavra, a
chapel, a feast day or a battleship named for a saint the subject never met.**
Taken unread they would have made Alexander Nevsky the associate of six
twentieth-century martyrs and John the Theologian the associate of five more.

`scripts/related-from-links.mjs` proposes and does not write, the standing
`place-candidates.mjs` has. Two things about it are worth keeping:

**Adjacency, not proximity.** The first version looked for "church" or
"monastery" anywhere in the forty characters before the name and set aside "of
the Trinity-Sergius monastery, sent with Andrew Oslyabya to the battle by
Sergius of Radonezh" — the one relation in the corpus nobody would want to lose.
A dedication runs *straight into* the name: the word, then at most "of", "of
the", "of St". Two more shapes came off the first clean run — "the Trinity and
Alexander Nevsky lavras" (plural) and "the John Chrysostom chapel" (the word
after, not before).

**And then a table, because no rule was going to catch a warship.** `REFUSED`
holds two by hand with the reason written beside each: the *Sviatoi Ioann
Zlatoust*, whose 1912 mutiny Roman Medved calmed, and Alexander Nevsky's "God is
not in strength but in truth", quoted seven centuries later by Nicholas of
Alma-Ata. That is Amendment 44's "the matching is a table rather than a rule",
met in a second place.

62 rows across 59 folders. The test pins both halves — Athanasius of Vysotsk,
who went to Makovets to be Sergius's disciple, has the link and the row;
Zenobius (Mazhuga), who spent thirty-five years at the Alexander Nevsky church,
has the link and no row.

### Continue reading, on a phone

The Daily page has carried the reader's own two shelves since the shelves
existed and a saint's page carried nothing under the life: past 1024 px the
apparatus column beside it holds their whole search, and below that width the
column is not drawn at all, so a phone reached the end of a life and the page
stopped. `mountShelves` is borrowed rather than a third list of saints written
for this page.

Three things it had to get right, and each is a bug avoided rather than a
preference. It sits **outside the late box**, which is hidden until the payload
lands and which the error path replaces wholesale — a shelf inside either would
vanish along with a life that failed to arrive, which is exactly the moment a
reader most wants a way onward, and it is the reader's own record and owes
nothing to this saint's folder. It is **mounted on the media query rather than
hidden by CSS**, on the same `WIDE` the column itself is placed on, so a desk
pays neither the two IndexedDB reads nor the store subscription. And it **drops
the saint whose page it is**: opening a saint records the visit, so the first
version offered the reader Continue reading and the row was the page they were
standing on.

### What is left, and the two things to know before starting it

**383 hymn objects still have no English.** The plan is `docs/HYMN-PLAN.md` and
its first item is unchanged by the reversal and still the cheapest: **no
kontakion in the corpus has ever been matched against Orloff's commons**, and he
prints one in every general service. A citation beats a rendering wherever one
exists, so that pass should run before any more translating.

**Two things noticed and left alone**, both the author's call. An English hymn
still prints its tone as "глас 4", quoted from the source — which sits oddly
under "when you select English … it should be in English", but the tone is part
of the citation. And a saint page's `related` is one-directional by design: the
instruction was that saints named in *their* life appear in *their* list, and
the reverse claim is one the life does not make.
...
## Amendment 105 — the header row moved to the rung it fits on, the tone put into the reader's language, and a language audit (2026-09-07, night)

CI went red on Amendment 104 and the author reported three things beside it:
saints with no name in the other languages ("St John Chrysostom, St John the
Theologian ... Neagoe Basarab has no name in Russian"), Greek hymns printing
"Glasul 3", and a request for "a full sweep of language equivalencies across
all content".

### The red, which was the fifth label and not the fifth label's fault

`ru at 560: the page overflowed by 10`. Five nav labels do fit one wide header
row at 560 px in this desk's Segoe UI, which is why the amendment went out
green from here; on the runner's own DejaVu Sans the Russian row ran the page
10 px off its side. Amendment 24's lesson, in the header for the third time: a
width measured in the native face is a fact about one machine.

Trimming the nav's gap — the fix that got it through here — was treating the
symptom. Measured properly, in Verdana at 560 and scaled for DejaVu, the
widest pack's row wants about **680 px before the masthead gets a single
pixel**, and Greek and Serbian were only inside 560 at all because the name
had already been squeezed to nothing: the name's track is the one a shortfall
comes out of, and it had given everything it had. So the wide row now starts
where it fits, at the site's own **760 rung** — the map's `isDesktop`, the
two-column layouts, the ladder calendar.css writes down — and everything under
it gets the strip, which is what the strip is for and why a fifth page needed
one. The gap went back to space-2. Six blocks in base.css, three `matchMedia`
calls in main.js, two constants in chrome.spec.js. Checked at every rung in
all five packs with the wide face forced: no overflow anywhere, and the
masthead keeps its full 91 px at 760 instead of 0.

**The deploy is why the author saw none of Amendment 104.** The quality-floor
step failed, so the `deploy` job was skipped and `6139267` never reached the
site — which is the whole of "St Anthony of the Caves is not hyperlinked in St
John the Long-Suffering's profile, and he isn't under RELATED either". Both
have been in the folder and in `HEAD` since that commit and both render here.
They are pinned now (`saint.spec.js`), because that pair is the case
`lib/cross-link.js` structurally cannot reach — the life says "Anthony of the
Caves" and the folder is `Anthony of the Kyiv Caves`, one word apart, and a
linker that bridged that gap is a linker that makes two saints one person.
`related-from-links.mjs` cannot watch it either: it reads *unlinked* prose, so
the moment the link exists the sweep steps over it. A test is the only thing
left that would notice it going.

### The tone, which was the one word in the line still speaking Slavonic

A hymn quotes its tone from the calendar it was read out of, so it arrives in
that calendar's notation — «глас 4», `Glasul 3`, `Ἦχος πλ. α´` — and the page
printed it as it stood. Everything around it was already the reader's: the
kind, the church, the name. So a reader in Greek met a Romanian troparion under
`Glasul 3`, which is what the author reported, and a reader in English met
«глас 4», which is the open item HANDOFF had twice left to the author. The
author has now called it.

`lib/tone.js` reads the Octoechos number out of any of the three notations —
including the plagals, which are the half of Greek notation that is not a
numeral (`πλ. α´` is 5, `βαρύς` is the grave seventh) — and the pack says the
word through the `tone` string it already had. **416 of the corpus's 419 tones
parse.** The three that do not are `Ἦχος πλ.`, a plagal whose numeral the
source itself never printed, and they keep the source's own string: guessing
which plagal would be inventing a fact about a hymn. Translating a number
between three spellings of it edits no quotation, which is why this is a
reversal that can be made at all.

Two tests pinned the defect and now pin the fix, and one of them pinned it
exactly: a Romanian calendar read in Greek, which is the author's sentence in
a test written a fortnight before he sent it.

### The sweep, and the two standards it has to report

`scripts/language-audit.mjs`. It reports and never writes, the standing
`place-candidates.mjs` and `related-from-links.mjs` already have.

What it found first was that **the question has two honest answers**, and that
the disagreement between them is the whole of the work:

| | a: named where a church that keeps them speaks it | b: named in every language the site reads in |
| --- | --- | --- |
| ru | 549 → **563/567 (99%)** | 563/862 |
| ro | 149 → **154/160 (96%)** | 154/862 |
| el | 352 → **361/365 (99%)** | 370/862 |
| sr | 116 → **120/129 (93%)** | 121/862 |

(The first number in each is where this sitting found it, the second where the
reading below left it.)

`lib/saint-name.js` was written to (a) and says so in its header — "where a
language has no form, the reason is usually not a gap" — and by that standard
the corpus is nearly finished. The author's own instruction of 2026-08-26,
quoted at the top of that same file, is (b): "every saint name needs to have
the equivalent in the displayed language." **27 saints of 862 have all four.**

The gap between them is not an oversight anybody can sweep up. A saint has a
Russian name because a Russian calendar names him; Neagoe Basarab is kept by
the Romanian church alone, so no Russian source in this repository names him,
and the only ways to (b) are a source nobody here has read or a
transliteration — which would be generated saint data, and is the author's
call rather than a script's. That is the decision this audit exists to put.

**What could be closed without one: 47 rows where the name was already sitting
inside a citation the saint's own folder carries, unread.** `--list propose`
lifts them out; **29 were read and written this sitting**, across 25 folders,
and that is the whole of the improvement in the table above. Every row needed
a reading, and the script says so twice: a Slavonic calendar lists its saints
in the *genitive* («Свт. Иоанна Златоустого» is "of John Chrysostom"), and the
rank and the see it prints are their own fields here. So «Прп. Антония
Великого» is `Антоний Великий`, and «Праведных Богоотец Иоакима и Анны» — a
pair, in the genitive — is `Иоаким` on one page and `Анна` on another.

**Eighteen were refused, and they are the argument for the tool proposing
rather than writing.** A citation names the *day*, not always the saint filed
under it: «Άγιος Βαβύλας … και τα Τρία Παιδιά» is offered for each of the
three children and names none of them; „Св. 70 мученика.“ is offered for three
envoys of Nicomedia and names nobody at all; „Св. муч. Антуса, и прочи с њом.“
names Anthousa, whose servants are the saints being asked about. Of the
thirteen Serbian rows exactly four name their own saint. A `--write` would
have put another saint's name on nine pages.

The Serbian pack is also why the extractor is worth a line: **pravoslavno.rs
closes with the mark English opens with** — „…“ — so a closing class of
`[»”"]` matched none of its thirteen, and the pack read as though its
citations carried no names at all. It was the last quotation mark of the four
calendars to be got right.
...
and `pickNameForms` refuses a form naming a company where the saint is one
person, correctly: printing it would put another saint's name on this saint's
page. So the corpus has looked better covered in Greek than it is, by about a
...
- **A Greek synaxarion writes a Western saint as «Άγιος Albeus»** — a Greek
  honorific around a Latin name — and by majority that form is Latin. What is
  a defect is a form with *none* of its language's own letters, not one with
  fewer than half.
- **Basilica publishes the Romanian Patriarchate's calendar in English**, and
  a script test cannot tell "†) St. Anthony the Great" from Romanian. Six
  English lines were proposed as Romanian name forms; a Romanian line has to
  say so in its own words now — a diacritic, or the rank the calendar leads
  with.
- **A string with no words in it** — `{label} - {name}`, `{credit} · {licence}`
  — is identical in every pack because there is nothing there to translate,
  which was the whole of the "echoing English" finding.

The one real mismatch it found is Shushanik's «Вардандухт», her Armenian given
name filed as `hy` and written in the Cyrillic the Russian source renders it
in. **Relabelling it `ru` was tried and reverted the same minute**, and the
reason is worth keeping: `pickNameForms` takes the *longest* form per
language, so the second Russian entry displaced «Шушаника» as the name a
Russian reader is shown — a rename, out of a tidy-up. It sits in the audit's
own `ACCEPTED` table with its reason instead, which is Amendment 44's "the
matching is a table rather than a rule" in a third place.

**Left for the author.** The (b) standard, which is the transliteration
question above and governs some 2,200 name slots. The 34 proposals, which are
a reading. 16 hymns whose tone is thin — 13 with none recorded and the three
truncated plagals. And John the Theologian and Neagoe Basarab specifically:
both are named in the message, and neither folder holds a source in the
language being asked for, so both are (b) rather than something a sweep can
reach.

## Amendment 106 — the phone's nav made endless in fact, the second Anthony linked, and the offices put into the reader's language (2026-09-08)

Three requests, one message, on the strip Amendment 105 had just moved to the
760 rung.

### The strip, which was three findings and two repairs

"First it needs to be an infinite horizontal scroll, next to Daily page on the
left needs to be the About section, and when you select one it should be
animated to click into the centre gently instead of just jumping with no
smoothness."

**The first two are one repair, and the first was already claimed done.**
Amendment 104 shipped `ui/nav-scroll.js` describing itself as endless, and it
was — but only *after* a swipe. It rotated when a gesture settled with an edge
page already centred, so at rest the current page stood at one end of the five
with blank strip beside it: on the Daily page, nothing at all to its left. The
loop existed and the reader had to earn it. The claim in that file's own header
was true of the mechanism and false of the page, which is the second time this
sitting a piece of prose has been right about code and wrong about what a
reader meets.

Now every settle — and the first paint — turns the ring until whichever page is
centred stands in the **middle** of the five. There are two pages either side at
all times, so the ring's own neighbours are the ones on screen, and About is
left of Daily because the ring says so rather than because a rotation happened
to land there.

**The rebalance is visually silent by construction**, which is the whole reason
it can run on every settle rather than only at an edge: rotating a ring does not
change anybody's neighbours, so re-centring the same page afterwards puts all
five back on the pixel they were already on. No compensating arithmetic, and
nothing to get wrong in a frame where two layouts are in play — which is what
the old `insertBefore` + `scrollLeft` nudge was.

**And the rotation is a flex `order`, not a DOM move.** The first cut moved the
nodes. It worked, and it cost two things worth more than it: `.site-nav a` came
back in an order that depended on where the reader had last swiped — so every
positional selector in the suite, and every keyboard user's tab order, became a
function of a gesture — and the compensation had to be exact. An `order` per
link says the same thing with neither. `offsetLeft`, which is all that file
reads, already answers in visual order. The trade is that focus order is the
canonical order rather than the rotated one; for a ring of five equals with no
meaningful sequence between them that is the better of the two, and it is the
desktop row's order besides.

**The gentle press needed three things, and two of them were not the animation.**

First, the row must survive the navigation. It was rebuilt from `innerHTML` on
every route change, so the pressed page arrived in a strip with no history —
nothing to glide *from*. `renderNav` now moves `aria-current` in place on a
phone and rewrites the labels (which is what keeps a language change honest on
that path), and rebuilds only when the row itself changes shape.

Second, `padding-inline` went from `calc(50vw - 3rem)` to a round `50vw`. The
old value left about 48 px short — more than half of most of these labels — so
the strip's own first and last page could never quite be centred, and a glide to
one of them would have landed short with the rebalance behind it showing as
exactly the jump being removed.

Third, **the glide cannot start inside the view transition**, and this was
measured rather than reasoned about. `renderNav` runs inside
`startViewTransition`'s update callback, where the browser has suspended
rendering and is about to cover the page with its snapshots. A `scrollTo({
behavior: 'smooth' })` started there moved the strip **one pixel** and stopped;
the same call outside a transition travels the whole way over nine frames.
`restoreSection`'s own note names that suspension from the other side, which is
what said where to look. So the glide is armed in `renderNav` and let go from
`show()` once `finished` settles — beside `settleLate`, which is there for a
kindred reason.

Native smooth scrolling rather than a hand-rolled tween, and the reason is the
snap: the strip is `scroll-snap-type: x mandatory`, and a per-frame `scrollLeft`
write into a mandatory-snap scroller is re-snapped under the animation.
`scrollTo` is the one motion the snapping cooperates with. Reduced motion gets
the centre and not a quicker journey to it.

Two tests, both backed out before being kept. `the phone strip is balanced at
rest, and a press glides into the centre` reads the *visual* order (About left of
Daily) and asserts the DOM's is untouched, then counts the distinct positions the
track passes through per frame — ten with the glide, three with it backed out,
and a jump is two. `under reduced motion the strip is simply centred, with no
journey` is the same instrument reading fewer than three. Backing out the
balance alone fails the order assertion; backing out the glide alone fails the
count. `COLD_FACE=1` green in both projects.

### The second Anthony

"In St John the Long-Suffering there is a mention of the caves of Anthony. Make
sure Anthony hyperlinks to Anthony of the Caves as well."

The life's last sentence closes on "the Near Caves, the caves of Anthony" — the
bare given name, thirty lines below the full one Amendment 105 pinned. It is
further out of `lib/cross-link.js`'s reach than the first, not nearer: `Anthony`
alone is three saints in this corpus, and a linker that resolved it would be a
linker that makes three saints one person. So it is hand-written, like the first,
and `saint.spec.js` now holds both — the count and each one's own text, since a
count alone would pass with the wrong link twice.

The house rule that a name is linked on its first appearance and left alone
after is a rule about a page a reader arrives at the top of. This is the last
sentence of a long life; the author has called it, and the reversal is recorded
rather than absorbed.

### The offices, and the sweep that could not have found them

"Offices, e.g. 'Princess' or 'Abbot' etc. not translated to other languages. I
had asked for a full sweep of all content to check for translation misses, im
disappointed it wasnt all translated."

**The sweep that missed them was a sweep of strings, and an office is not a
string in this repository.** Amendment 105's `language-audit.mjs` walks the
packs and the saints' *name forms*; `locale-coverage.mjs` walks `ui/strings.js`.
An `office` is a field in 337 saints' own files, and nothing that reads `ui/`
could ever have seen it. That is why the check written here walks the corpus:
the same reasoning that missed it once would have missed it again.

`lib/calendar-page.js`'s `formatSubtext` carried the decision this reverses, in
its own words: the office "is printed as the corpus recorded it, which is
English — 'Bishop of Voronezh' is a fact about a see, not a UI string, and the
packs cannot hold 340 of them." The first half is true and does not decide it: a
see has a name in Russian and in Greek, and printing the English one on an
otherwise Russian page is not deference to the record, it is a gap. The second
half was arithmetic, and wrong. **The corpus's 337 offices are 139 distinct
phrases, built from 38 ranks and 96 places**, and a pack holds them without
noticing.

Composed rather than hand-listed, from a rank table and a place table, so a
phrase is translated once wherever it occurs — but the *output* is what went
into the packs, whole strings keyed on the English, because composition is
where the grammar goes wrong. Russian and Serbian take the adjectival see-form
their calendars use («Епископ Никомидийский», «Архиепископ цариградски»), Greek
takes the genitive throughout, Romanian "de" for a city and "al" for a region;
and the one feminine office that takes a place at all — Queen of Georgia — takes
a genitive, which is gender-blind in all four. A composed string joins on the
comma with the second part lowercased, which is how a calendar sets it:
«Апостол от семидесяти, епископ Сардийский».

**The 23 attestation titles go through the same table**, because they are the
same kind of phrase from the same kind of source, on the same line — `Bishop of
Hippo` is one entry whichever field it arrived in. The *deduplication* stays in
English on purpose: `views/saint.js` compares recorded strings, which is where
two churches saying the same thing is visible, and comparing translations would
make that test depend on whether a pack rendered two different English phrases
alike.

`offices` is the packs' **second pack-only branch**, after `reasons`, and for
the same reason: its keys are English phrases, so there is no English entry for
them to be the base of. Three places had to know that and only one did —
`pruneTo`, `tests/i18n.test.mjs`'s two structural sweeps, and
`locale-coverage.mjs` — so the list is `lib/i18n.js`'s `PACK_ONLY` now rather
than `'reasons.'` spelled out three times. The cost of forgetting one of them is
a report full of false findings, which is how a real one gets missed.

Two instruments, and the second caught something. `locale-coverage.mjs` reports
offices with no entry per pack (0 of 158 in all four), as a report and not a
gate, the same bargain `date-audit.mjs` makes about a missing birth year — a new
saint with an untranslated see is the ordinary way this goes stale. The unit
test also refuses a pack that merely **repeats the English**, and that is what
found Romanian `Novice`, which is a real Romanian word and a thin translation of
a Russian «послушник»; it reads `Ascultător` now, which is what Basilica prints.

Each pack grew about 2.4 kB gzipped (ru 11.25 → 13.60) and the entry chunk by
0.30. The §13 gate fails on this desk on **both** trees — clean 1537/1588/1559
against 1541/1689/1557 with the change, on FCP samples whose own spread is
wider than the difference — so it is the desk, as HANDOFF has said twice, and
CI is the machine whose answer counts.

### Left for the author: the dates on the same line

Reading the office line in five languages showed the *next* miss of the same
kind, and it is not this instruction's. The lifespan beside the office prints
its interval's recorded `display` string verbatim — English months, English
ordinals, English prose: «Архиепископ Константинопольский · c. 347 по Р. Х. –
**14 September** 407 по Р. Х.», and «Игумен · Кончина - **13th C.**».

Measured rather than estimated: 438 distinct display strings over 1,060
intervals, of which **73 contain real words, on 152 intervals**. They are three
families and only one of them is a judgement call:

- **62 century ordinals** (`1st century`, `late 4th century`, `mid-8th
  century`), the most-seen by far and purely mechanical.
- **5 full dates** (`14 September 407`), which `Intl` already knows how to say.
- **33 era descriptions** (`under Diocletian`, `at the Council of Ephesus`,
  `under King Milutin`, `around the end of the first millennium`), which are
  the judgement call — quoted source-shaped prose, and `formatInterval`'s
  refusal to touch a `display` string is a recorded decision, not an oversight.
...
## Amendment 107 — the lifespans translated, the strip full while it moves, and the carousel's lag measured before it was guessed at (2026-09-08, evening)

Four requests in one message, on the work of the same afternoon.

### The lifespans, which were the second half of a line already half fixed

"Change lifespans to translated." Amendment 106 had put the *office* into the
reader's language and left the dates beside it in English, and named the gap
with its numbers rather than closing it — the author read the numbers and
called it. So a Russian page had gone from all-English on that line to
«Архиепископ Константинопольский · c. 347 по Р. Х. – **14 September** 407 по Р.
Х.», which is arguably worse than either end: a half-translated line reads as a
mistake where a fully English one reads as a limitation.

**Parsed, not tabulated, and that is the opposite of the choice made for the
offices the day before.** The two corpora have different shapes and the shape
decides. An office is 139 fixed phrases and the 140th is unpredictable, so a
table is the only honest answer. A date display is 438 strings built from a
vocabulary of about fifteen words where almost all the variety is *numbers* — a
table would want a new row for every year the corpus ever records.
`lib/date-display.js` reads the grammar and the packs supply the words.

The grammar, entire: an optional `probably`; an optional `BC`/`AD` that belongs
to the *whole* string and is therefore stripped once at the top (in "7th-6th
century BC" it is not either century's); `before`/`after`; a century with an
optional `early`/`late`/`mid-`; a century range; a full date; a year or year
range with an optional `c.`; and `or`, which is read **right to left** because
the right half carries the unit — "4th or 5th century" says century once and
means it twice.

Three things are worth keeping about how it went:

- **`under Hadrian or Antoninus` has an `or` in it that is not a join**, and
  splitting on it hands the second half over as a bare name nothing can read.
  A recorded phrase is tried *whole* before it is taken apart now.
- **`early 1900s` was silently wrong and the test did not catch it**, because
  the test asked only whether the answer differed from the English. It was
  going through the century patterns and coming out «начало 1900-е гг. в.» — a
  decade wearing a century's abbreviation. What found it was printing all
  eighty-one worded displays in five languages and reading them. `readDecade`
  is gone: the corpus has exactly one decade and it is a recorded phrase now,
  which is both correct and one code path fewer that nothing exercises.
- **`tests/i18n.test.mjs` refused the first design and was right to.** Each
  language wants a different *numeral* — Roman for ru/ro/sr, Arabic for el, an
  ordinal for en — and the first cut handed all three tokens to every pattern
  and let each pack pick. That breaks the check which holds a translation to
  the same placeholders as its English base, which is the check that catches a
  dropped `{name}`. `dates.centuryNumeral` names the style in one key instead,
  every pattern has the one `{n}`, and the check still applies.

**The era mark is decided on the English and applied to the translation.**
"Reposed 105 AD" but "Reposed 1937" is a rule every clause of which reads an
English shape — it already says AD, it ends in a digit or in "C." — and «IV в.»
ends in neither. Decided before, applied after; `withEra2` is that split and
says so.

**Conservative by construction**: anything it cannot account for *whole* comes
back exactly as recorded. `tests/date-display.test.mjs` then walks the saints'
own folders and holds every display the corpus actually contains to being
read, in all four packs — so the fallback stays a statement about a *new* shape
rather than a silence. It reads the folders and not `data/manifest.json`, which
is the lesson Amendment 106's own red CI run taught: `/data/` is gitignored and
`npm test` runs before `build:manifest`.

`eras` is the one table — nineteen reigns and councils, "under Diocletian", "at
the Council of Ephesus" — because a ruler's name is a proper noun in a
construction Slavonic declines («при Диоклетиане»). It is the third pack-only
branch, and `PACK_ONLY` is now doing the job it was named for.

### The strip, which was right about the arrangement and wrong about the frames

"Dont make the header text load after its settled and centred after selection.
They should be visible as the animation is happening, true infinite scroll."

Amendment 106 balanced the ring on every *settle*. Measured frame by frame, a
press on About from the Daily page opened **85 px of blank strip** on the
leading edge, and Texts appeared there only once the glide had landed: the row
ran out and refilled, in front of the reader, exactly as reported.

Turning the ring *before* the movement was the obvious fix and it was not
enough — it traded a leading gap of 85 px for a trailing one of 64. The real
finding was underneath: **the five labels measured 339 px inside a 360 px
window.** The whole ring was shorter than the screen, so blank showed at one
end or the other at every position, balanced or not, and no amount of turning
could have hidden it. `min-width: 28vw` makes the ring 140vw against the 128vw
that "window plus one page" needs, at every width, since both sides are in vw.

With the ring long enough, the rule that keeps it full is the same during a
movement as at rest — whatever is nearest the midline sits in the middle of the
five — applied every frame. `turnKeepingStill` pins what the reader is looking
at, so the turn is invisible; `keepEndless` runs it on every scroll event of a
swipe, and the glide runs it per frame. **Blank edge during a press: 85 px to
zero, and the chrome test measures exactly that number now.**

That forced the glide to be hand-rolled, since a `scrollLeft` write aborts a
native smooth scroll, with snap taken off for the length of the tween. The
distance is carried as a *remainder* rather than two endpoints, which is what
makes a moving destination safe: a turn changes `centreOf(target)` by a whole
period and moves the picture not at all, so "how far the target still is from
the midline" is invariant across it. A pre-turn to the destination was tried
first and fights `keepEndless` on the very next frame.

**The cost taken, written where it is taken:** `keepEndless` writes `scrollLeft`
inside a live gesture, which can cut iOS momentum short. The first cut refused
to do that and paid for it with the blank edge.

### The carousel, where the reported cause and the real one were different things

"The carousel load is now quite laggy, especially on mobile. Cap the fade ins
and number of loading saints/images so they flicker in more slowly and it lags
less." And: "Would smaller thumbnail images help as well? If so make them."

**The pictures are 5% of it.** Eight seconds of All Saints at 4x CPU on a 360 px
screen: **1,830 ms of blocked main thread**. Blocking every card image outright
— through `serviceWorkers: 'block'`, because `page.route` sees none of these
(trap 13, and the request count is what said the first attempt had failed open)
— moved it to **1,738**. Slicing the run from 215 columns to 64 moved it to
**680**. The cost is the number of columns laid out, near enough linearly, and
the row carries the whole corpus in 215 of them by design.

`content-visibility: auto` on `.cx-cell` answers exactly that and nothing else:
**1,830 → 570 ms, worst task 880 → 470, dropped frames 21-41 → zero**, with
every column still there. It is safe here because this row's geometry is
*declared* rather than discovered — every cell is `--cx-w` wide and sits exactly
at its `min-height` — so `contain-intrinsic-size` states what the browser would
have computed anyway. Checked rather than assumed: widths, heights and
`offsetLeft`s with the property on and off, identical to the pixel, which
matters because `loopScroll` reads its wrap period off `offsetLeft` and a row
whose period moved would drift off its own buffer.

**Four smaller cuts of real waste were made first and none of them moved the
total**, which is worth more than the four of them together. Said plainly:

- `ui/coachmark.js` listens for scrolls on `document` in the capture phase, so
  it took *every frame of the carousel's own drift* and read `scrollTop` in it —
  a forced layout, on a frame the drift had already dirtied, and 460 ms of the
  profile, second in the page. A horizontal scroller's `scrollTop` is 0 for ever
  besides, so every one of those was a comparison that could not change.
- The drift read `scrollLeft` back twice a frame; it reads it once and hands it
  on.
- `frozen()` asked for `clientWidth` sixty times a second to answer "is this
  track laid out"; a `ResizeObserver` answers from the frame the browser has
  already done.
- `nameLines` re-measured all 862 names on every repaint; it memoises.

Each is a genuine reduction, each was profiled, and the eight-second total
after all four was 1,707–1,879 against a baseline of 1,738–1,835 — inside the
run-to-run noise. **The profile moved and the sum did not**: fixing the
coachmark simply handed the forced layout to whichever reader came next.

And what was actually asked for is done on its own merits: the fade-ins are
queued 120 ms apart so the row fills one picture at a time rather than in
fours, and a phone loads two at once rather than four. `imageInflight` says in
its own note that this is a look and a data bill rather than a frame budget.

**The thumbnails: not for the lag, but the bytes are real.** A phone draws a
150 CSS px card and the derivative it draws is 560 px on its long edge — 3.5x
the pixels a DPR-2 phone can show. A 320 px derivative would be ~15 kB against
48, and the first screenful of All Saints on a phone is about twelve pictures:
**579 kB now, ~189 kB then.** Not made here, because the question was asked
about the lag and the answer to that is measured at 5%; it is a data-plan
...
## Amendment 108 — the strip answers the press rather than the navigation, and the atlas gets out of the corpus's way (2026-09-08, late)

### The header, which was three complaints and one cause

"Still dont like the header on mobile though I feel like I want the animation
to be separate from the loading below otherwise it feels disjointed uneven and
unpredictable. It should be a smooth instant response, and the loading below
should happen independently. Also you should be able to see the other 2 header
buttons even if its just half of them, and then have a fade on the edges so it
looks like they're fading out into the edges of the screen."

**The glide was armed by the navigation and let go by the navigation.**
Amendment 107 started it from `show()` once `startViewTransition(...).finished`
had settled, which was itself a fix for a real thing — a smooth scroll begun
inside the transition callback moves one pixel and dies — but it made the
header's answer wait on the page. Measured: **277 ms between the press and the
strip moving**, and the wait is the *page's* length, so it is different every
time. Three events for one touch, in an order the reader cannot predict, which
is exactly "disjointed uneven and unpredictable".

Two changes, and the second is what makes the first possible:

- The glide starts **on the press**, in the nav's own click listener, before
  the router is asked for anything. **36 ms**, and the page below changes at
  **30 ms** — neither waiting for the other, which is the whole request.
- The cross-fade is skipped for this one gesture. A view transition replaces
  the document with a snapshot for its duration, so a header animating
  underneath one is a header nobody can see; there is no arrangement of the two
  where both the fade and a live header exist. Only this press — every other
  navigation on the site still cross-fades — because this is the only one where
  the chrome is the thing the reader is looking at.

*The wider fix, considered and not taken: `:root { view-transition-name: none }`
with the name moved to `#view` scopes the cross-fade to the content and leaves
the chrome live for every navigation. It also rewrites the saint page's own
shared-element and swipe transitions, which is a great deal of risk to buy a
fade on the one gesture that reads better without it.*

**And `min-width` went 28vw to 25vw, which is arithmetic rather than taste.**
With the centred page on the midline, the pages two along start at `1.5w` from
centre and the screen ends at `V/2`, so a reader sees `(V/2 - 1.5w)/w` of each:
28% at 28vw, 42% at 26vw, **half at exactly `V/4`** — 25vw, at every width,
since both terms are in vw.

26vw was tried first and looked right in the number and wrong on the screen,
which is the finding worth keeping: **half the box and half the word are the
same thing only at 25vw.** A label is centred in its box, so the 42% that
showed at 26vw was the box's *outer* edge and held only the last few letters —
13 px of a 40 px "Map". The screenshot is what said so; the percentage did not.

The cost is the movement margin. No gap at rest needs `2.5w >= V/2`, which 25vw
clears by 45 px; no gap at the worst point of a movement needs
`2.5w >= V/2 + w/2`, which 25vw meets exactly with nothing to spare. That is
affordable only because of the mask, which fades the outer 9% to nothing: a
hairline opening at the very edge mid-glide opens inside the band that cannot
be seen. The suite still measures the geometric gap at 0, because
`keepEndless` turns the ring at the halfway point and the worst case is
therefore the boundary rather than past it.

**The edge fade is a mask and not two overlays**, because an overlay would have
to be painted in `--gesso` and this row sits on two themes and on a bar that is
translucent at its own edge; a mask takes the pixels out and lets what is
behind stay behind. **9% and not 16%**: at 25vw the outermost page shows 12.5%
of the screen, and a ramp across all of it leaves every pixel of that half-word
under half opacity — a smudge, which is what the first cut produced. At 9% the
ramp is 32 px of a 45 px sliver, so the word fades as it runs off the edge
rather than starting the run already faint.

### The atlas layer, where the author's diagnosis was wrong and the instruction was right

"On the map noone's title is showing from Nicomedia anymore. I think its
because it would clash with the city name NICOMEDIA on the map. Please make
these city names on a different layer to the other text so its okay if they
clash and place them to the left of a dot instead of to the right."

**The mechanism guessed at is not there and the layer is already separate.**
`HISTORICAL_LABELS` has never been in `obstacles`; it cannot take space from a
saint's name and `layoutLabels` has never heard of it. Read at the search's own
flight zoom, seven saints are named at Nicomedia on a desktop and five on a
phone — nothing is being dropped.

**What is there is a collision, and the instruction fixes it exactly.**
`layoutLabels` tries the right of a dot first and takes it whenever it is free,
so every saint name in the picture starts to the right of its own dot — and so
did every city name, out of the same coordinate. The screenshot is the whole
report: NICOMEDIA drawn through "Martyr Adrian of Nicomedia +26", and
CONSTANTINOPLE through "Martyr Agathonicus of Nicomedia". Two legible things in
one space read as neither, which is what "noone's title is showing" is about.

Sending this layer left costs nothing — it is not laid out against anything, so
it has no preference to give up — and empties the side the corpus wants. Both
city names are clear of both saint names now, at the same zoom, in the same
picture.

It is still allowed to clash, which is the arrangement rather than a gap in it.
What keeps an overlap legible where one happens is that the two are not the
same *kind* of type: capitals, tracked, a third smaller and at a third of the
ink, against mixed case at full strength. An atlas prints a region across a
city for the same reason.

**The test for it was written twice, and the first one was an instrument that
could not see its own subject.** It published the anchor the pass had used and
asserted the anchor was left of the marker — which passed with the draw
reverted, because a revert does not have to touch the variable the anchor is
published from. It publishes the *alignment* as well now: the anchor alone
misses a `textAlign` flipped back to `left`, the alignment alone misses an
offset flipped back to `+5`, and only the pair catches both. Found the way this
file's own rule says to find it, by backing the change out and watching the
test pass.

### Postscript: two instruments that could not see their own subject, in one sitting

**The suite caught the fade skip, and it was right to.** `a returning Daily page
lands where it was left` reads `window.scrollY` at the transition's own `ready`
— the moment the fade is composed — and it presses the strip's Daily button to
get there. That press no longer runs a transition, so the reading came back
`NaN` in both projects. The promise it measures belongs to the section restore
and not to either button, so the instrument moved to the masthead, which goes to
the same place and still fades; and the strip's own path is now checked by where
it *lands*, which is what a reader on that route actually cares about.

**And two of this sitting's own tests were blind in the same way.** Both were
found by backing the change out and watching the test pass, which is this
codebase's own rule and the only thing that finds them:

- The atlas test published the *anchor* the pass had used and asserted it was
  left of the marker. Reverting the draw to the right kept passing, because the
  anchor is published from a variable a revert does not have to touch. It
  publishes the **alignment** as well now: the anchor alone misses a `textAlign`
  flipped back, the alignment alone misses an offset flipped back, and only the
  pair catches both.
- The strip test asserted the row started moving within 120 ms of the press.
  That reads `scrollLeft`, which is the *DOM* — and a view transition covers the
  document with a snapshot for its duration, so with the fade put back the strip
  would move on time and the reader would watch a still picture. It counts the
  transitions the press starts as well now, and each half fails on its own when
  the other is left in place.

Three in one sitting, on a page whose own instructions say to ask what an
instrument would look like if it were doing nothing. The common shape: **all
three read a number the code publishes about itself rather than the thing the
reader sees.** A published anchor is not a drawn glyph and a `scrollLeft` is not
a painted frame. Where a canvas or a snapshot stands between the state and the
reader, one assertion is almost never enough.

**A fourth thing, about the run rather than the code.** A backgrounded
`npm run test:all` reported "880 passed" and exited 0, and the tail was all this
session read. The full log said **17 failed** — the summary line had been
truncated out of the captured window. Two of those were real (the transition
test above); the other fifteen were fifteen leftover browser processes from an
afternoon of profiling, and passed on a quiet machine. Both halves of that are
worth keeping: a truncated pass is not a pass, and a spread of failures across
unrelated surfaces is a fact about the desk before it is a fact about the diff.
